...
export const useEditorStore = create<EditorStore>()(
  (set, get) => ({
    ...
    updateCellColor: (row, col, color) =>
      set((state) => {
        ...
        if (cell && typeof cell === 'object' && ('type' in cell) && (cell.type === 'stone' || cell.type === 'frozen')) {
          newGrid[row][col] = { ...cell, color }
        }
        return {
          grid: newGrid,
          activeLevel: { ...state.activeLevel, grid: newGrid },
          isDirty: true,
        }
      }),
    ...
    saveLevel: () => {
      ...
      get().saveWorlds().catch(e => console.error("Auto-save failed", e))
    },
    loadLevel: (levelId) =>
      set((state) => {
    ...
    placePlayPiece: (row, col) => {
      const state = get()
      if ((!state.playMode && !state.solutionMode) || !state.selectedPlayPieceId || state.playMovesLeft <= 0) {
    ...
    toggleSolutionMode: () =>
      set((state) => {
        const nextMode = !state.solutionMode
        if (nextMode) {
          return {
            solutionMode: true,
            playMode: false,
        ...
        if (data.worlds && data.worlds.length > 0) {
          ...
          if (firstLevel) {
            set({
              ...
              activeLevelId: firstLevel.id,
              activeLevel: firstLevel,
              grid: firstLevel.grid.map((r: any) => [...r]),
              history: [firstLevel.grid.map((r: any) => [...r])],
              historyIndex: 0,
              initialized: true,
            })
          } else {
            set({ worlds: data.worlds, initialized: true })
          }
        } else {
